...
| -------- | ------- | ---- |
| V1.0 | intitial revision before implementation of M2.3  | 2026-05-14
...
A StateGraph is a directed graph of StateNodes connected by Transitions.
...
SimulationCore does not persist the full StateGraph definition in M2.3/M2.4.
The StateGraphPath is derived from TerminalNodeType according to the frozen StateTree definition.
...
Astronometria is the execution entity of astronomical simulations in AstronoSphere. Astronometria supports three major operation modes. 
...
> Note: A Scientific Mode knows about its identity ("I am supposed to execute a ScientificRun now") from the input: an Experiment and its corresponding GroundTruth .
...
The future Astronometria Ephemeris Engine will be a StateGraph. Every StateNode in this StateGraph is generating a unique and reproducible intermediate result. The **TerminalNode** contains 
...
- the delta between Astronometria StateVector of the TerminalNode and the Ground Truth shall be calculated.
...
A SimulationRun can be any of these Runs: ScientificRun, ExplorationRun, StatisticalRun.
...
### 4.2.4 Example for GroundTruth
   ...
   ```json
  "GroundTruthRef": {
    "Provider": "Horizons", -> derived from FactorMetadata.Source
...
If not stated differently: Default for Truncation is "none".

### 4.2.6 Example for Engine Citation
  ...
  ```
> Note: Citation blocks are cumulative through the AstronoSphere pipeline.

### 4.2.7 Example for Provenance
  ...
  ```
> Note: Provenance blocks are cumulative through the AstronoSphere pipeline.
...
Position and Velocity objects SHOULD be written on a single line in SimulationData samples, following the GroundTruth formatting style.

Example:
```json
{
  "JD": 2443872.745666667,
  "Position": {"X":-0.46085412360236139,"Y":0.55010351196900875,"Z":0.034096085092839817},
...
1) Experiment
2) Corresponding GroundTruth


For this, the algorithm shall scan in the GT-File for the CatalogNumber. With the Catalog Number, the corresponding Experiment shall be loaded. Cross-Check for M2.3/2.4 with the DatasetHeader.Measuement.

The following resolution rule shall be applied:

```text
GT-Resolution:
ExperimentRef.CatalogNumber == Experiment.CatalogNumber
AND
ExperimentRef.CoreHash == Experiment.CoreHash
AND
DatasetHeader.Measurement == VEC/L0
```
...
StateHash includes the canonical form of 
> RunEnvironment + SceneContext + Target + NodeType + JD + numerical policy
...
GroundTruth: AstronoData\03_GroundTruth\Ephemeris\Horizons\Baseline
...
3) PHYS.L1.HELIO.ECL.J2000.VEC
4) PHYS.L1.GEO.ECL.J2000.VEC
5) PHYS.L2.HELIO.ECL.J2000.VEC
6) PHYS.L2.GEO.ECL.J2000.VEC
...
ScientificRun requires deterministic GroundTruth resolution.

Astronometria scans the GroundTruth files and resolves exactly one matching GroundTruth dataset by means of:
- CatalogNumber
- CoreHash
- Measurement
...
To cover these cases count unequal to 1, a new Diag-Family must be introduced:
...
```text
040.002
Multiple matching GroundTruth datasets found - Warning

040.003
Invalid ExperimentMaturity - Warning

040.004
Provider Range violation - Warning

040.008 
Unsupported ScientificRun configuration

040.009
No matching GroundTruth dataset found - Warning
```
...
> **Rule**
If Metadata.Status.Maturity != Released:
→ write 040.003
→ do not resolve GroundTruth
→ do not derive TerminalNode
→ do not execute simulation
→ do not write SimulationData

> **Rule:**
First matching diagnostic condition wins.
Exactly one diagnostic record per experiment per run.

## 9.3 Folder structure for diagnostic codes
...
The logic to move Run -> LastRun shall be the same as for valid data sets.
...
## 10.3 Example JSON Draft Venus L0 at Observer Time

This is an example for a Scientific Venus simulation for Experiment AS-000003.
Note that only the first and the last calculated data set is displayed, indicated by ... in between.
    ...
    "TargetSimulations": [
      {
        ...
        "TerminalNode": {
          "NodeId": "VEN_NODE_001",
          "NodeType": "PHYS.L0.HELIO.ECL.J2000.TDB.VEC",
